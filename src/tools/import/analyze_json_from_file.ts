        ...
        async ({ path, sample_size = 10 }) => {
            ...
            try {
                ...
                try {
                    ...
                    return createErrorResponse(
                        `File not found or not readable: ${trimmedPath}`,
                    );
                }
                ...
                if (![".json", ".jsonl", ".ndjson"].includes(ext)) {
                    debugLog(
                        `Warning: File extension ${ext} is not typically JSON`,
                    );
                }
                ...
                try {
                    ...
                    if ([".jsonl", ".ndjson"].includes(ext)) {
                        ...
                        if (jsonData.length === 0) {
                            return createErrorResponse(
                                "No valid JSON objects found in JSONL file",
                            );
                        }

                        // For JSONL, we'll analyze as an array of objects
                        debugLog(
                            `Parsed ${jsonData.length} JSON objects from JSONL file`,
                        );
                    } else {
